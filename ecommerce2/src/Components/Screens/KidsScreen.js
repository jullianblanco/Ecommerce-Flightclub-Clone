...
const ProductDataItems = ProductData.filter(item => item.category === 'kids').map(item => {
   return {
     id: item.id,
     img: item.img,
     title: item.title,
  ...
  const KidsProducts = ProductDataItems.map((ProductDataItems) =>
 <div key={ProductDataItems.id}>
  <Link to={`/sneakers/products/${ProductDataItems.id}`} style={{textDecoration: 'none'}}>  
     <div className='Product'> 
       <img className='product img' src={ProductDataItems.img} alt='shoe'/>
...
function KidsScreen() {
    return (
  <div className="KidsScreen">
   <NavList /> 


        <h1 className='sneakers-h1'>Kids</h1>
